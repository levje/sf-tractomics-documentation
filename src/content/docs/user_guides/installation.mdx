...
## Install the pipeline for offline use
...
<Steps>
  1. **(Recommended)** Point the `NXF_HOME` environment variable in your scratch directory. When pulling or running a pipeline by only specifying its name
     ```bash
