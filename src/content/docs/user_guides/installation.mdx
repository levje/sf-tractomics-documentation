...
`sf-tractomics` integrates **a lot** of different tools together. To orchestrate the tools into a comprehensible and useful pipeline, we use third-party software that allow easy and efficient parallelization of tasks across subjects. Installing the prerequisite softwares below are the only things you'll need to install to be able to operate the entire pipeline.
...
<Aside type="note" title="Running on a HPC cluster">
  If you are planning on running the pipeline on a HPC server where compute nodes do not have internet access, a few more installation steps might be required (e.g. pre-downloading the apptainer images used by the pipeline) to ensure the pipeline can be ran properly. Please consult [this section to setup your environment for offline usage](/sf-tractomics/user_guides/offline).
</Aside>
...
## Next steps
Once everything installed and/or available, you should be well equiped to run the pipeline easily. You can now proceed to the next section where we detail how you should format your inputs before we detail how to properly run the pipeline.
