...
<Aside type="caution">
   If you're trying to run the pipeline running on an HPC cluster and you haven't downloaded and installed the containers to run the pipeline offline, we suggest doing so here before running the pipeline [as detailed here](/sf-tractomics/user_guides/offline).
